---
status: active
---

# Active backlog

- [ ] [Phase 1: Record experiment boundary](items/phase-1-experiment-boundary.md)

## Authorization prerequisite

- [ ] [Enforce owner authorization before enterprise session transfer](items/enforce-enterprise-owner-authorization.md)

No further real organization-managed session export or replay may proceed until this high-priority precondition is met. It applies independently of phase order.

## Upcoming phases

- [ ] [Phase 2: Prove extension-only transport](items/phase-2-extension-transport-gate.md)
- [ ] [Phase 3: Resolve application compatibility](items/phase-3-application-compatibility-gate.md)
- [ ] [Phase 4: Cut over scoped control contract](items/phase-4-scoped-control-cutover.md)
- [ ] [Phase 5: Implement host application preparation](items/phase-5-host-application-preparation.md)
- [ ] [Phase 6: Complete Outlook web](items/phase-6-outlook-completion.md)
- [ ] [Phase 7: Complete Slack web](items/phase-7-slack-completion.md)
- [ ] [Phase 8: Complete Zoom Web App](items/phase-8-zoom-completion.md)
- [ ] [Phase 9: Prove integrated failure matrix](items/phase-9-integrated-failure-matrix.md)
- [ ] [Phase 10: Finish demonstrated cutover](items/phase-10-demonstrated-cutover.md)

Only Phase 1 is in the active queue. Phases 2 and 3 become parallel-safe after Phase 1; Phases 4–10 are sequential. The original transport and application-handoff backlog items remain supporting detail linked from the corresponding phase items.

Completed setup work (2026-09-06): [five-character browser pairing](../2026-09-06.short-code-pairing/index.md) is deployed to the personal Cloudflare account and verified with two real Chrome profiles. One typed code and bilateral confirmation replace descriptor copying; code-free reconnect and forget preserve the direct-WebRTC/Slack authorization boundaries. Physical Mac/Linux and the original application gates below remain open.

Application status (2026-09-06): Slack now has an evidence-backed existing-session controller and a successful same-machine, two-profile extension WebRTC handoff. Client account/workspace/enterprise and original host preservation passed; occupied-client overwrite was rejected. Physical Mac/Linux, Case 1 and the complete failure matrix remain open, so Phase 7 is not closed. [Slack evidence](../2026-09-05.lan-login-handoff/slack-session-transport-experiment.json). Outlook remains paused at its authentication boundary; [resume report](../2026-09-05.lan-login-handoff/research-outlook-session-transport.md).
